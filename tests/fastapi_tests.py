...
@pytest.mark.asyncio
async def test_transfer_between_users():
    async with httpx.AsyncClient() as client:
...
async def create_external_wallet_and_withdraw(client, user_token, service_token, amount=0.1):
    ...
    response = await client.post(f"{BASE_URL}/wallet/withdraw", headers=deposit_headers, params={
        "service_user_id": service_token,  # предположим, что это ID сервисного пользователя
...
@pytest.mark.asyncio
async def test_wallet_operations_with_service_user():
    async with httpx.AsyncClient() as client:
