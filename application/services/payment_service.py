...
from domain.repositories.payments_repository import update_transaction
...
async def update_service_transaction(session: AsyncSession, transaction_id: uuid.UUID, new_status: str, service_user_id: uuid.UUID):
    new_transaction_service = await update_transaction(session, transaction_id, new_status, service_user_id)
